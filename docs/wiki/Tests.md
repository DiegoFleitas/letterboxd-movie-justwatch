...
Configuration: root [`vitest.config.ts`](https://github.com/DiegoFleitas/letterboxd-movie-justwatch/blob/master/vitest.config.ts) (Node environment, `APP_SECRET_KEY` for sessions).

**SonarCloud:** Quality metrics use **[Automatic Analysis](https://docs.sonarsource.com/sonarqube-cloud/advanced-setup/automatic-analysis/)** in SonarCloud (bound to this GitHub repo). There is **no** Sonar scanner in CI (it cannot run alongside Automatic Analysis). [`sonar-project.properties`](https://github.com/DiegoFleitas/letterboxd-movie-justwatch/blob/master/sonar-project.properties) documents scope for the SonarCloud project; keep `sonar.organization` and `sonar.projectKey` aligned with the project in SonarCloud.
