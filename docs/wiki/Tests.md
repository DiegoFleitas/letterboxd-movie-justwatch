...
| `bun run test:coverage`                                                          | Same Vitest run with **v8 coverage** (`text`, `lcov`, `html` under `coverage/`; CI feeds `coverage/lcov.info` to [SonarCloud](https://sonarcloud.io/project/overview?id=DiegoFleitas_letterboxd-movie-justwatch)) |
...
**SonarCloud:** After [`Unit tests with coverage`](https://github.com/DiegoFleitas/letterboxd-movie-justwatch/blob/master/.github/workflows/ci.yml), CI runs [`SonarSource/sonarqube-scan-action`](https://github.com/SonarSource/sonarqube-scan-action) using [`sonar-project.properties`](https://github.com/DiegoFleitas/letterboxd-movie-justwatch/blob/master/sonar-project.properties). Add a [SonarCloud](https://sonarcloud.io) project bound to this repo (or import from GitHub), then create a **project analysis token** and store it as **`SONAR_TOKEN`** in GitHub **Settings → Secrets and variables → Actions**. Ensure `sonar.organization` and `sonar.projectKey` in `sonar-project.properties` match the SonarCloud project (organization key is lowercase in SonarCloud; see README Sonar badges for the project id).
