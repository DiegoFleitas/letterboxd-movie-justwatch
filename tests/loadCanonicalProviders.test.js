/**
 * Tests for loadCanonicalProviders: format handling and getters (using _injectForTest).
 */
...
suite.test("getCanonicalProviderMap and getCanonicalProviderByNames return injected new format", () => {
  ...
  const byClearName = { "HBO Max": { id: "max", name: "HBO Max" }, "HBO Max  Amazon Channel": { id: "max", name: "HBO Max" } };
...
suite.test("getCanonicalProviderByNames returns empty when only byTechnicalName injected", () => {
  _resetCache();
  _injectForTest({ byTechnicalName: { netflix: { id: "netflix", name: "Netflix" } } });
